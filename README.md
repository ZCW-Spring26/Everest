...
To create a double-clickable macOS `.app` bundle, run:

```bash
mvn clean package -Pmacos -DskipTests
```

This produces `target/Everest-Alpha-1.4-macos.zip`. Extract it and you will find `Everest.app`.

**Before launching**, make sure JavaFX is available. The easiest way is via [Homebrew](https://brew.sh):

```bash
brew install openjfx
```

The launcher will automatically detect JavaFX installed by Homebrew (both Apple Silicon and Intel).
Alternatively, set the `JAVAFX_HOME` environment variable to your JavaFX SDK directory, or place
the JavaFX SDK in `Everest.app/Contents/Java/javafx/`.
